...
public final class WebhooksApi extends BaseApi {
    ...
    /**
     * Re-sends the webhook payload for a previously delivered (or failed) event. Returns 202
     * Accepted immediately; delivery is asynchronous.
     * @param  storeId  Required parameter: The unique identifier of the store.
     * @param  id  Required parameter: The unique identifier of the resource.
     * @param  eventId  Required parameter: The unique identifier of the webhook event.
     * @param  idempotencyKey  Optional parameter: An optional idempotency key to prevent double
     *         charges and duplicate operations. We recommend a randomly generated UUID (v4).
    ...
    public ApiResponse<Object> redeliverWebhookEvent(
            ...
            final UUID eventId,
            final String idempotencyKey) throws ApiException, IOException {
        return prepareRedeliverWebhookEventRequest(storeId, id, eventId, idempotencyKey).execute();
    }
     ...
     * @return    Returns the Object wrapped in ApiResponse response from the API call
     */
    public CompletableFuture<ApiResponse<Object>> redeliverWebhookEventAsync(
            final UUID storeId,
            final UUID id,
            final UUID eventId,
            final String idempotencyKey) {
        try {
            return prepareRedeliverWebhookEventRequest(storeId, id, eventId, idempotencyKey).executeAsync();
        } catch (Exception e) {
            throw new CompletionException(e);
        }
    }
    ...
    /**
     * Builds the ApiCall object for redeliverWebhookEvent.
     */
    private ApiCall<ApiResponse<Object>, ApiException> prepareRedeliverWebhookEventRequest(
            final UUID storeId,
            final UUID id,
            final UUID eventId,
            final String idempotencyKey) {
        return new ApiCall.Builder<ApiResponse<Object>, ApiException>()
                ...
                .requestBuilder(requestBuilder -> requestBuilder
                        ...
                        .templateParam(param -> param.key("id").value(id)
                                .shouldEncode(true))
                        .templateParam(param -> param.key("eventId").value(eventId)
                                .shouldEncode(true))
                        .headerParam(param -> param.key("Idempotency-Key")
                                .value(idempotencyKey).isRequired(false))
                        .headerParam(param -> param.key("accept").value("application/json"))
