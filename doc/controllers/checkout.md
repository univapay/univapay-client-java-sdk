...
```java
CompletableFuture<ApiResponse<CheckoutInfo>> getCheckoutInfoAsync()
```
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` getter of this instance returns the response data which is of type [`CheckoutInfo`](../../doc/models/checkout-info.md).

## Example Usage

```java
checkoutApi.getCheckoutInfoAsync().thenAccept(result -> {
