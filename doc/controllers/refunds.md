...
```java
CompletableFuture<ApiResponse<RefundList>> listRefundsAsync(
    final UUID storeId,
    final UUID chargeId,
    final Integer limit,
    final UUID cursor,
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` getter of this instance returns the response data which is of type [`RefundList`](../../doc/models/refund-list.md).
...
refundsApi.listRefundsAsync(storeId, chargeId, limit, cursor, cursorDirection, metadata).thenAccept(result -> {
...
```java
CompletableFuture<ApiResponse<Refund>> createRefundAsync(
    final UUID storeId,
    final UUID chargeId,
...
UUID storeId = UUID.fromString("0cab399b-5621-425b-993b-f8507eba1e78");
UUID chargeId = UUID.fromString("6efb4e5c-690a-40f3-a4f1-0e19c5f84e98");
RefundCreateRequest body = new RefundCreateRequest.Builder(
    1000,
    "JPY"
)
...
refundsApi.createRefundAsync(storeId, chargeId, body, idempotencyKey).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
| `polling` | `Boolean` | Query, Optional | If `true`, the server holds the connection open until the refund status transitions from `pending` to a terminal state, or until the polling timeout is reached. |
...
refundsApi.getRefundAsync(storeId, chargeId, id, polling).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
```java
CompletableFuture<ApiResponse<Refund>> updateRefundAsync(
    final UUID storeId,
    final UUID chargeId,
    final UUID id,
...
UUID chargeId = UUID.fromString("6efb4e5c-690a-40f3-a4f1-0e19c5f84e98");
UUID id = UUID.fromString("c4e87129-cad4-47fb-8ded-b4c0a4ae0dd4");
RefundUpdateRequest body = new RefundUpdateRequest.Builder()
...
refundsApi.updateRefundAsync(storeId, chargeId, id, body, idempotencyKey).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
