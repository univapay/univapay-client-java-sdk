...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` getter of this instance returns the response data which is of type [`TransactionToken`](../../doc/models/transaction-token.md).
...
transactionTokensApi.listAllTransactionTokensAsync(limit, cursor, cursorDirection).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
transactionTokensApi.listStoreTransactionTokensAsync(storeId, limit, cursor, cursorDirection).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
transactionTokensApi.getTransactionTokenAsync(storeId, id).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
