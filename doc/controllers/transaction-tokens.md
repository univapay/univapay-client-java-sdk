...
```java
TransactionTokensApi transactionTokensApi = client.getTransactionTokensApi();
```
...
```java
CompletableFuture<ApiResponse<TransactionToken>> createTransactionTokenAsync(
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` getter of this instance returns the response data which is of type `TransactionToken`.

## Example Usage

```java
TransactionTokenCreateRequest body = new TransactionTokenCreateRequest.Builder(
    ...
    TransactionTokenCreateRequestData.fromTokenCreateCardData(
        ...
        .cvv("123")
        .phoneNumber(new TokenCreatePhoneNumber.Builder(
                "81",
                "08012341234"
            )
            .build())
        .cvvAuthorize(new TokenCreateCardDataCvvAuthorize.Builder()
                .enabled(false)
                .currency("JPY")
                .build())
        .threeDs(new TokenCreateCardDataThreeDs.Builder()
...
transactionTokensApi.createTransactionTokenAsync(body, null).thenAccept(result -> {
    // TODO success callback handler
    result.getResult().match(new TransactionToken.Cases<Void>() {
        @Override
        public Void cardTransactionToken(CardTransactionToken cardTransactionToken) {
            System.out.println(cardTransactionToken);
            return null;
        }

        @Override
        public Void konbiniTransactionToken(KonbiniTransactionToken konbiniTransactionToken) {
            System.out.println(konbiniTransactionToken);
            return null;
        }

        @Override
        public Void onlineTransactionToken(OnlineTransactionToken onlineTransactionToken) {
            System.out.println(onlineTransactionToken);
            return null;
        }

        @Override
        public Void bankTransferTransactionToken(BankTransferTransactionToken bankTransferTransactionToken) {
            System.out.println(bankTransferTransactionToken);
            return null;
        }

        @Override
        public Void paidyTransactionToken(PaidyTransactionToken paidyTransactionToken) {
            System.out.println(paidyTransactionToken);
            return null;
        }

        @Override
        public Void qrScanTransactionToken(QrScanTransactionToken qrScanTransactionToken) {
            System.out.println(qrScanTransactionToken);
            return null;
        }

        @Override
        public Void qrMerchantTransactionToken(QrMerchantTransactionToken qrMerchantTransactionToken) {
            System.out.println(qrMerchantTransactionToken);
            return null;
        }
    });
...
```java
CompletableFuture<ApiResponse<TransactionTokenList>> listAllTransactionTokensAsync(
    final String search,
...
| `customerId` | `UUID` | Query, Optional | Filter by customer ID. |
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` getter of this instance returns the response data which is of type [`TransactionTokenList`](../../doc/models/transaction-token-list.md).

## Example Usage

```java
String search = "tokyo";
UUID customerId = UUID.fromString("8a3f1b8e-2c1a-4b7a-9c2e-6f6b6f6e2b10");
...
transactionTokensApi.listAllTransactionTokensAsync(search, customerId, type, mode, active, limit, cursor, cursorDirection).thenAccept(result -> {
    // TODO success callback handler
...
```java
CompletableFuture<ApiResponse<TransactionTokenList>> listStoreTransactionTokensAsync(
    final UUID storeId,
    final String search,
    final UUID customerId,
    final TransactionTokenListType type,
...
transactionTokensApi.listStoreTransactionTokensAsync(storeId, search, customerId, type, mode, active, limit, cursor, cursorDirection).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
```java
CompletableFuture<ApiResponse<TransactionToken>> getTransactionTokenAsync(
    final UUID storeId,
    final UUID id,
...
| `polling` | `Boolean` | Query, Optional | If set to true, instructs the API to internally poll the token's 3DS or CVV authorization sub-status until it transitions to another status, or until the ~3 second server-side timeout is reached. |
...
transactionTokensApi.getTransactionTokenAsync(storeId, id, polling).thenAccept(result -> {
    // TODO success callback handler
    result.getResult().match(new TransactionToken.Cases<Void>() {
        @Override
        public Void cardTransactionToken(CardTransactionToken cardTransactionToken) {
...
```java
CompletableFuture<ApiResponse<TransactionToken>> updateTransactionTokenAsync(
    final UUID storeId,
    final UUID id,
    final String idempotencyKey,
...
UUID storeId = UUID.fromString("0cab399b-5621-425b-993b-f8507eba1e78");
UUID id = UUID.fromString("c4e87129-cad4-47fb-8ded-b4c0a4ae0dd4");
TransactionTokenUpdateRequest body = new TransactionTokenUpdateRequest.Builder()
...
transactionTokensApi.updateTransactionTokenAsync(storeId, id, null, body).thenAccept(result -> {
    // TODO success callback handler
    result.getResult().match(new TransactionToken.Cases<Void>() {
        @Override
        public Void cardTransactionToken(CardTransactionToken cardTransactionToken) {
...
```java
CompletableFuture<ApiResponse<Void>> deleteTransactionTokenAsync(
    final UUID storeId,
    final UUID id)
```
...
transactionTokensApi.deleteTransactionTokenAsync(storeId, id).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
```java
CompletableFuture<ApiResponse<TransactionToken>> enableTokenThreeDsAsync(
    final UUID storeId,
    final UUID id,
    final String idempotencyKey,
...
UUID storeId = UUID.fromString("0cab399b-5621-425b-993b-f8507eba1e78");
UUID id = UUID.fromString("c4e87129-cad4-47fb-8ded-b4c0a4ae0dd4");
EnableTokenThreeDsRequest body = new EnableTokenThreeDsRequest.Builder()
    .redirectEndpoint("https://univapay.com/3ds-redirect")
    .build();

transactionTokensApi.enableTokenThreeDsAsync(storeId, id, null, body).thenAccept(result -> {
    // TODO success callback handler
    result.getResult().match(new TransactionToken.Cases<Void>() {
        @Override
        public Void cardTransactionToken(CardTransactionToken cardTransactionToken) {
...
```java
CompletableFuture<ApiResponse<TransactionToken>> disableTokenThreeDsAsync(
    final UUID storeId,
    final UUID id)
```
...
transactionTokensApi.disableTokenThreeDsAsync(storeId, id).thenAccept(result -> {
    // TODO success callback handler
    result.getResult().match(new TransactionToken.Cases<Void>() {
        @Override
        public Void cardTransactionToken(CardTransactionToken cardTransactionToken) {
...
```java
CompletableFuture<ApiResponse<ThreeDsIssuerToken>> getTokenThreeDsIssuerTokenAsync(
    final UUID storeId,
    final UUID id)
```
...
transactionTokensApi.getTokenThreeDsIssuerTokenAsync(storeId, id).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
