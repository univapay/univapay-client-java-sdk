...
```java
ChargesApi chargesApi = client.getChargesApi();
```
...
```java
CompletableFuture<ApiResponse<Charge>> createChargeAsync(
...
```java
ChargeCreateRequest body = new ChargeCreateRequest.Builder(
...
.metadata(new GenericMetadata.Builder()
        .orderId("12345")
        .build())
.redirect(new ChargeCreateRequestRedirect.Builder()
        .endpoint("https://test.url/")
        .build())
.build();

chargesApi.createChargeAsync(null, body).thenAccept(result -> {
...
```java
CompletableFuture<ApiResponse<ChargeList>> listAllChargesAsync(
    final Integer limit,
    final UUID cursor,
...
| `expMonth` | `Integer` | Query, Optional | Filter by expiration month.  **Note:** If specified, `last_four`, `name`, and `exp_year` must also be included. |
| `expYear` | `Integer` | Query, Optional | Filter by expiration year.  **Note:** If specified, `last_four`, `name`, and `exp_month` must also be included. |
...
| `amountFrom` | `Integer` | Query, Optional | Show charges with an amount greater than or equal to this value. |
| `amountTo` | `Integer` | Query, Optional | Show charges with an amount strictly less than this value. |
...
| `transactionTokenId` | `UUID` | Query, Optional | Filter by transaction token ID. |
...
UUID transactionTokenId = UUID.fromString("f33b673e-564c-4645-ae17-ca03846a86b7");

chargesApi.listAllChargesAsync(limit, cursor, cursorDirection, lastFour, name, expMonth, expYear, from, to, email, phone, amountFrom, amountTo, currency, mode, metadata, transactionTokenId).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
```java
CompletableFuture<ApiResponse<ChargeList>> listStoreChargesAsync(
    final UUID storeId,
    final Integer limit,
    final UUID cursor,
    final CursorDirectionQuery cursorDirection,
...
chargesApi.listStoreChargesAsync(storeId, limit, cursor, cursorDirection, lastFour, name, expMonth, expYear, from, to, email, phone, amountFrom, amountTo, currency, mode, metadata, transactionTokenId).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
```java
CompletableFuture<ApiResponse<Charge>> getChargeAsync(
    final UUID storeId,
    final UUID id,
...
| `polling` | `Boolean` | Query, Optional | If set to true, instructs the API to internally poll the charge status  until it changes from 'pending' (the initial status) to another status. |
...
chargesApi.getChargeAsync(storeId, id, polling).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
```java
CompletableFuture<ApiResponse<Charge>> updateChargeAsync(
    final UUID storeId,
    final UUID id,
    final String idempotencyKey,
...
UUID storeId = UUID.fromString("0cab399b-5621-425b-993b-f8507eba1e78");
UUID id = UUID.fromString("c4e87129-cad4-47fb-8ded-b4c0a4ae0dd4");
ChargeUpdateRequest body = new ChargeUpdateRequest.Builder()
    .metadata(new GenericMetadata.Builder()
        .orderId("1234")
        .build())
    .build();

chargesApi.updateChargeAsync(storeId, id, null, body).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
```java
CompletableFuture<ApiResponse<IssuerToken>> getChargeIssuerTokenAsync(
    final UUID storeId,
    final UUID id)
```
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` getter of this instance returns the response data which is of type [`IssuerToken`](../../doc/models/issuer-token.md).
...
chargesApi.getChargeIssuerTokenAsync(storeId, id).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
```java
CompletableFuture<ApiResponse<ThreeDsIssuerToken>> getChargeThreeDsIssuerTokenAsync(
    final UUID storeId,
    final UUID id)
```
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` getter of this instance returns the response data which is of type [`ThreeDsIssuerToken`](../../doc/models/three-ds-issuer-token.md).
...
chargesApi.getChargeThreeDsIssuerTokenAsync(storeId, id).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
```java
CompletableFuture<ApiResponse<BankTransferLedgerList>> listBankTransferLedgersAsync(
    final UUID storeId,
    final UUID id)
```
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` getter of this instance returns the response data which is of type [`BankTransferLedgerList`](../../doc/models/bank-transfer-ledger-list.md).
...
chargesApi.listBankTransferLedgersAsync(storeId, id).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
```java
CompletableFuture<ApiResponse<CustomsDeclarationWebhookData>> createCustomsDeclarationAsync(
    final UUID storeId,
    final UUID chargeId,
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` getter of this instance returns the response data which is of type [`CustomsDeclarationWebhookData`](../../doc/models/customs-declaration-webhook-data.md).
...
CustomsDeclarationCreateRequest body = new CustomsDeclarationCreateRequest.Builder(
    "TOKYO",
    "1234567890",
    "AB1234567",
    "TARO YAMADA"
...
chargesApi.createCustomsDeclarationAsync(storeId, chargeId, body, idempotencyKey).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
```java
CompletableFuture<ApiResponse<CustomsDeclarationWebhookData>> getCustomsDeclarationAsync(
    final UUID storeId,
    final UUID chargeId,
    final UUID id,
    final Boolean polling)
...
| `storeId` | `UUID` | Template, Required | The unique identifier of the store. |
| `chargeId` | `UUID` | Template, Required | The unique identifier of the charge. |
| `id` | `UUID` | Template, Required | The unique identifier of the customs declaration. |
| `polling` | `Boolean` | Query, Optional | Hold the request open while waiting for a status change.<br><br>**Default**: `false` |
...
UUID storeId = UUID.fromString("0cab399b-5621-425b-993b-f8507eba1e78");
UUID chargeId = UUID.fromString("6efb4e5c-690a-40f3-a4f1-0e19c5f84e98");
UUID id = UUID.fromString("11ef0000-0000-4000-8000-000000000040");
Boolean polling = false;

chargesApi.getCustomsDeclarationAsync(storeId, chargeId, id, polling).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
```java
CompletableFuture<ApiResponse<CustomsDeclarationWebhookData>> patchCustomsDeclarationAsync(
    final UUID storeId,
    final UUID chargeId,
    final UUID id,
...
UUID chargeId = UUID.fromString("6efb4e5c-690a-40f3-a4f1-0e19c5f84e98");
UUID id = UUID.fromString("11ef0000-0000-4000-8000-000000000040");
CustomsDeclarationPatchRequest body = new CustomsDeclarationPatchRequest.Builder(
    "1234567891"
)
.build();
...
chargesApi.patchCustomsDeclarationAsync(storeId, chargeId, id, body, idempotencyKey).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
