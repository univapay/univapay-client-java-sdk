...
```java
CompletableFuture<ApiResponse<Merchant>> getCurrentMerchantAsync()
```
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` getter of this instance returns the response data which is of type [`Merchant`](../../doc/models/merchant.md).

## Example Usage

```java
merchantsApi.getCurrentMerchantAsync().thenAccept(result -> {
