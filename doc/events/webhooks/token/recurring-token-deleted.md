    ...
    public ResponseEntity<String> receiveEvent(
        ...
        String result = TokenHandler.parseEventAsync(httpRequest).thenApply(tokenParsingResult ->
            tokenParsingResult.matchSome(new TokenParsingResult.SomeCases<String>() {
                @Override
                public String recurringTokenDeleted(TokenWebhookEvent recurringTokenDeleted) {
                    return MessageFormat.format("RecurringTokenDeleted event received {0}", recurringTokenDeleted.toString());
                }
