    ...
    public ResponseEntity<String> receiveEvent(
        ...
        String result = SubscriptionHandler.parseEventAsync(httpRequest).thenApply(subscriptionParsingResult ->
            subscriptionParsingResult.matchSome(new SubscriptionParsingResult.SomeCases<String>() {
                @Override
                public String subscriptionSuspended(SubscriptionWebhookEvent subscriptionSuspended) {
                    return MessageFormat.format("SubscriptionSuspended event received {0}", subscriptionSuspended.toString());
                }
